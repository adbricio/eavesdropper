...
from tno.quantum.communication.qkd_key_rate.protocols.classical.winnow import WinnowCorrector,WinnowReceiver,WinnowSender
...
def information_reconciliation(alice_key, bob_key, error_rate):
   message_length = len(alice_key)
   input_message = Message(alice_key)
   error_message = Message(bob_key)
   ...
   corrected_Alice_key = np.array(corrector.alice._message.message)
   corrected_Bob_key = np.array(corrector.bob._message.message)

   return corrected_Alice_key, corrected_Bob_key, summary


def bb84_key_perc(desired_key_length, link_length, distance_to_alice, percentage, attenuation, redundancy_rate):
    ...
    qber_after_correction = None


    # If QBER test is passed, correct key
    if qber<0.11 and qber!=0:
        alice_final_key, bob_final_key, _ = information_reconciliation(alice_final_key, bob_final_key, qber)
        qber_after_correction = calculate_qber(alice_final_key, bob_final_key)
    

    return alice_final_key, bob_final_key, qber, qber_after_correction, simulated_time, repetitions
